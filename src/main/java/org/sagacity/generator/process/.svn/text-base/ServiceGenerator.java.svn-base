/*******************************************************
 * @author LIZHITAO
 * @date 2014-3-24 下午5:23:59
 * @name ServiceGenerator.java
 * @JDK version 1.6
 * @version 0
 ******************************************************/
...
public class ServiceGenerator extends Generator {
	...
	public void process(List<Table> list, String tableName) {
		...
		for(Table t : list){
			...
			root.put("package", Config.DEFAULT_SERVICE_PACKAGE);
			root.put("entityName", t.getName());
			root.put("entityNameCN", t.getNameCN());
			String fileName = t.getName() + "Service.java";
			try {
				Template template = getConfiguration().getTemplate("service.ftl");
				buildTemplate(root, fileName, Config.SERVICE_PATH, template);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	public void undoProcess(List<Table> list, String tableName) {
		if (null != tableName) {
			try {
				if (new File(Config.SERVICE_PATH + tableName
						+ "Service.java").exists()) {
					FileUtils.forceDelete(new File(Config.SERVICE_PATH
							+ tableName + "Service.java"));
				}
				if (new File(Config.SERVICE_PATH).exists()
						&& new File(Config.SERVICE_PATH).listFiles().length == 0) {
					FileUtils.deleteDirectory(new File(Config.SERVICE_PATH));
				}
			...
			for (Table t : list) {
				try {
					if (new File(Config.SERVICE_PATH + t.getName()
							+ "Service.java").exists()) {
						FileUtils.forceDelete(new File(Config.SERVICE_PATH
								+ t.getName() + "Service.java"));
					}
				} catch (IOException e) {
					e.printStackTrace();
				}
